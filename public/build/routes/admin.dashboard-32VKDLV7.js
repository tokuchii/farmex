import {
  db
} from "/build/_shared/chunk-F3PUUZFH.js";
import {
  require_news
...
import {
  collection,
  onSnapshot
} from "/build/_shared/chunk-ZMN6YE3T.js";
import {
  require_trainings
...
var require_with_selector_development = __commonJS({
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-shim/with-selector.development.js"(exports) {
    ...
    (function() {
      ...
      "undefined" !== typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ && "function" === typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.registerInternalModuleStart && __REACT_DEVTOOLS_GLOBAL_HOOK__.registerInternalModuleStart(Error());
      var React36 = require_react(), shim = require_shim(), objectIs = "function" === typeof Object.is ? Object.is : is4, useSyncExternalStore2 = shim.useSyncExternalStore, useRef18 = React36.useRef, useEffect18 = React36.useEffect, useMemo11 = React36.useMemo, useDebugValue2 = React36.useDebugValue;
...
var require_orderBy = __commonJS({
  "node_modules/es-toolkit/dist/compat/array/orderBy.js"(exports) {
    ...
    function orderBy(collection2, criteria, orders, guard) {
      if (collection2 == null)
        return [];
      orders = guard ? void 0 : orders;
      if (!Array.isArray(collection2))
        collection2 = Object.values(collection2);
...
var require_sortBy = __commonJS({
  "node_modules/es-toolkit/dist/compat/array/sortBy.js"(exports) {
    ...
    function sortBy6(collection2, ...criteria) {
      const length = criteria.length;
      if (length > 1 && require_isIterateeCall2.isIterateeCall(collection2, criteria[0], criteria[1]))
        criteria = [];
      else if (length > 2 && require_isIterateeCall2.isIterateeCall(criteria[0], criteria[1], criteria[2]))
        criteria = [criteria[0]];
      return require_orderBy2.orderBy(collection2, require_flatten2.flatten(criteria), ["asc"]);
    }
...
var require_use_sync_external_store_with_selector_development = __commonJS({
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-with-selector.development.js"(exports) {
    ...
    (function() {
      ...
      "undefined" !== typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ && "function" === typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.registerInternalModuleStart && __REACT_DEVTOOLS_GLOBAL_HOOK__.registerInternalModuleStart(Error());
      var React36 = require_react(), objectIs = "function" === typeof Object.is ? Object.is : is4, useSyncExternalStore2 = React36.useSyncExternalStore, useRef18 = React36.useRef, useEffect18 = React36.useEffect, useMemo11 = React36.useMemo, useDebugValue2 = React36.useDebugValue;
      exports.useSyncExternalStoreWithSelector = function(subscribe, getSnapshot, getServerSnapshot, selector, isEqual) {
        var instRef = useRef18(null);
        if (null === instRef.current) {
          var inst = { hasValue: false, value: null };
...
if (import.meta) {
  ...
  import.meta.hot.lastModified = "1782802393781.3604";
}
...
function toPHTime(iso) {
  const d = new Date(iso);
  return new Date(d.getTime() + 8 * 60 * 60 * 1e3);
}
function formatPHHour(hour) {
  if (hour === 0)
    return "12 AM";
  if (hour === 12)
    return "12 PM";
...
function buildTodayData(createdAts) {
...
function buildWeekData(createdAts) {
  const dayNames = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  const days = dayNames.map((name) => ({ label: name, visitors: 0 }));
  createdAts.forEach((iso) => {
    const d = new Date(iso);
    if (!isNaN(d.getTime()))
      days[d.getDay()].visitors++;
  });
  return days;
}
function buildMonthData(createdAts) {
  const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
  const data = months.map((label) => ({ label, visitors: 0 }));
  createdAts.forEach((iso) => {
    const d = new Date(iso);
    if (!isNaN(d.getTime()) && d.getFullYear() === (/* @__PURE__ */ new Date()).getFullYear()) {
      data[d.getMonth()].visitors++;
    }
  });
  return data;
}
function buildYearData(createdAts) {
  const currentYear = (/* @__PURE__ */ new Date()).getFullYear();
  const yearMap = /* @__PURE__ */ new Map();
  for (let y2 = 2024; y2 <= currentYear; y2++) {
    yearMap.set(y2, 0);
  }
  createdAts.forEach((iso) => {
    const d = new Date(iso);
    if (!isNaN(d.getTime())) {
      ...
      if (y2 >= 2024 && y2 <= currentYear) {
        yearMap.set(y2, (yearMap.get(y2) || 0) + 1);
      }
    }
  });
  return Array.from(yearMap.entries()).map(([y2, count]) => ({
    label: String(y2),
    visitors: count
  }));
}
function filterBySince(createdAts, since) {
  const sinceISO = since.toISOString();
  return createdAts.filter((iso) => iso >= sinceISO);
}
function useVisitorChart(view) {
  const [allCreatedAts, setAllCreatedAts] = (0, import_react49.useState)([]);
  const unsubRef = (0, import_react49.useRef)(null);
  (0, import_react49.useEffect)(() => {
    ...
    function subscribe() {
      ...
      unsubRef.current = onSnapshot(
        ...
        (snapshot) => {
          const dates = snapshot.docs.map((doc) => doc.data().createdAt).filter((v) => typeof v === "string" && v.length > 0);
  ...
  if (view === "today") {
    filtered = filterBySince(allCreatedAts, startOfDay(/* @__PURE__ */ new Date()));
  } else if (view === "week") {
    filtered = filterBySince(allCreatedAts, startOfWeek(/* @__PURE__ */ new Date()));
  }
  const chartData = view === "today" ? buildTodayData(filtered) : view === "week" ? buildWeekData(filtered) : view === "month" ? buildMonthData(filtered) : buildYearData(filtered);
...
if (import.meta) {
  ...
  import.meta.hot.lastModified = "1782802393789.7307";
}
  ...
  return /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)("section", { className: "space-y-6 rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-900/5", children: [
    ...
    /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)("div", { className: "rounded-3xl border border-slate-200 bg-white p-6 shadow-sm", children: [
      /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)("div", { className: "mb-6 flex items-center justify-between", children: [
        ...
        /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)("div", { className: "flex gap-1 rounded-xl border border-slate-200 bg-slate-50 p-1", children: filterTabs.map((tab) => /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)("button", { onClick: () => handleViewChange(tab.key), className: `rounded-lg px-3 py-1.5 text-xs font-semibold transition ${view === tab.key ? "bg-emerald-500 text-white shadow-sm" : "text-slate-600 hover:bg-slate-200"}`, children: tab.label }, tab.key, false, {
          fileName: "app/routes/admin.dashboard.tsx",
          lineNumber: 156,
          columnNumber: 36
        }, this)) }, void 0, false, {
      ...
      /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)("div", { className: "h-[350px]", children: /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(ResponsiveContainer, { width: "100%", height: "100%", children: /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(AreaChart, { data: chartData, accessibilityLayer: false, margin: {
        top: 20,
        right: 20,
        left: 0,
        bottom: 0
        ...
        /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)(XAxis, { dataKey: "label", axisLine: false, tickLine: false, tick: {
          fill: "#9ca3af",
          fontSize: 13
        } }, void 0, false, {
          fileName: "app/routes/admin.dashboard.tsx",
...
_s(AdminDashboard, "wWBN4ZhsaNaxbRgSkziu7TrKxCM=", false, function() {
  return [useLoaderData, useSearchParams, useVisitorChart, useAdminUrlToast];
});
_c = AdminDashboard;
...
react-is/cjs/react-is.development.js:
   ...
   *)
*/
//# sourceMappingURL=/build/routes/admin.dashboard-32VKDLV7.js.map
