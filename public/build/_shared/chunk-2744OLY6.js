...
var init_router2 = __esm({
  "node_modules/react-router-dom/node_modules/@remix-run/router/dist/router.js"() {
    ...
    splatPenalty2 = -2;
    isSplat2 = (s) => s === "*";
    ABSOLUTE_URL_REGEX$12 = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i;
    isAbsoluteUrl2 = (url) => ABSOLUTE_URL_REGEX$12.test(url);
    removeDoubleSlashes2 = (path) => path.replace(/\/\/+/g, "/");
    joinPaths2 = (paths) => removeDoubleSlashes2(paths.join("/"));
...
var ABSOLUTE_URL_REGEX$13 = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i;
var isAbsoluteUrl3 = (url) => ABSOLUTE_URL_REGEX$13.test(url);
function resolvePath3(to, fromPathname) {
  ...
  let pathname;
  if (toPathname) {
    if (isAbsoluteUrl3(toPathname)) {
      pathname = toPathname;
    } else {
      if (toPathname.includes("//")) {
        let oldPathname = toPathname;
        toPathname = removeDoubleSlashes3(toPathname);
        warning3(false, "Pathnames cannot have embedded double slashes - normalizing " + (oldPathname + " -> " + toPathname));
      }
      if (toPathname.startsWith("/")) {
        pathname = resolvePathname3(toPathname.substring(1), "/");
      } else {
        pathname = resolvePathname3(toPathname, fromPathname);
      }
    }
  } else {
    pathname = fromPathname;
...
var removeDoubleSlashes3 = (path) => path.replace(/\/\/+/g, "/");
var joinPaths3 = (paths) => removeDoubleSlashes3(paths.join("/"));
...
function createRouter3(init) {
  ...
  async function handleAction(request, location, submission, matches, isFogOfWar, opts) {
    ...
    if (isRedirectResult3(result)) {
      ...
      if (opts && opts.replace != null) {
        ...
        let location2 = normalizeRedirectLocation3(result.response.headers.get("Location"), new URL(request.url), basename, init.history);
        replace5 = location2 === state.location.pathname + state.location.search;
      }
      await startRedirectNavigation(request, result, true, {
        submission,
  ...
  async function startRedirectNavigation(request, redirect8, isNavigation, _temp2) {
    ...
    location = normalizeRedirectLocation3(location, new URL(request.url), basename, init.history);
...
function normalizeRedirectLocation3(location, currentUrl, basename, historyInstance) {
  let invalidProtocols = [
    "about:",
    "blob:",
    "chrome:",
  ...
  if (ABSOLUTE_URL_REGEX4.test(location)) {
    ...
    if (url.origin === currentUrl.origin && isSameBasename) {
      return removeDoubleSlashes3(url.pathname) + url.search + url.hash;
    }
  }
  try {
    let url = historyInstance.createURL(location);
...
@remix-run/react/dist/esm/index.js:
   ...
   *)
*/
//# sourceMappingURL=/build/_shared/chunk-2744OLY6.js.map
